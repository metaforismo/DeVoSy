# Decentralised Voting System (DeVoSy) 🗳️
...
- Then the likely voters connect to the same blockchain network and register to become a voter. Once the users successfully register, their respective details are sent/displayed in the admin's panel (i.e., verification page).
- The admin then will check if the registration information (blockchain account address, name, and phone number) is valid and matches with their record. If yes, then the admin approves the registered user making them eligible to take part and cast their respective vote in the election.
...
1. Install **truffle** and **ganache-cli** using node package manager (npm)
...
1. Clone this repository
   ...
   ```shell
   git clone https://github.com/arlbibek/DeVoSy.git
   cd DeVoSy
   ```
